...
from services.github_service import (
    ...
    create_branch, commit_file, create_pull_request
)
...
async def run_docs_pipeline(project_id: str, db: AsyncIOMotorDatabase, event_callback=None):
    ...
    try:
        ...
        # STEP 6: Deployer — Push docs to repo via PR
        ...
        if token:
            try:
                branch_name = f"engineops/docs-{uuid.uuid4().hex[:8]}"
                base_branch = repo_info.get("default_branch", "main")

                await create_branch(owner, repo, branch_name, token, base_branch)

                # Commit each doc page as a markdown file under docs/
                for page in doc_pages:
                    filename = page["title"].lower().replace(" ", "-").replace("/", "-")
                    file_path = f"docs/{filename}.md"
                    commit_msg = f"docs: add {page['title']} documentation [EngineOps]"
                    await commit_file(owner, repo, file_path, page["content"], branch_name, commit_msg, token)

                # Create PR
                pr_body = f"""## EngineOps Documentation Update

Auto-generated documentation for **{owner}/{repo}**.

### Pages Included
{chr(10).join(f'- `docs/{p["title"].lower().replace(" ", "-").replace("/", "-")}.md` — {p["title"]}' for p in doc_pages)}

### Partner Integrations
- **Unsiloed**: {partner_badges.get('unsiloed', {}).get('files_parsed', 0)} unstructured files parsed
- **Safedep**: {safedep_result.get('total_dependencies', 0)} dependencies scanned, {len(safedep_result.get('findings', []))} findings

*Generated by [EngineOps](https://engineops.dev) — AI-Native Engineering Operations*
"""
                pr_result = await create_pull_request(
                    owner, repo,
                    f"docs: EngineOps documentation ({len(doc_pages)} pages)",
                    pr_body, branch_name, base_branch, token
                )
                await notify(6, "completed", f"PR created: {pr_result.get('url', 'N/A')} with {len(doc_pages)} doc pages")
            except Exception as e:
                logger.error(f"Docs PR creation failed: {e}")
                await notify(6, "completed", f"Docs stored locally. PR creation failed: {str(e)[:200]}")
        else:
            await notify(6, "completed", f"Documentation stored ({len(doc_pages)} pages). No GitHub token — PR skipped.")
